...
def main():
    ...
    sphere = Sphere(Point(3, 0, -10), 2, Vector(1, 0, 0))  # Esfera 

    plane = Plane(Point(0, -1, 0), Vector(0, 1, 0), (0, 1, 0))  # Plano verde
    ...
    for i in range(height):
        for j in range(width):
            ...
            if sphere_intersection:
                image[i][j] = (255, 0, 0)  # Vermelho para esfera
            else:
                plane_intersection = plane.intersect(ray)
                if plane_intersection:
                    image[i][j] = (0, 255, 0)  # Verde para o plano

    write_ppm(image, width, height, "imagem.ppm")
    print("Imagem output.ppm")
